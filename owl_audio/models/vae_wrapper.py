...
def load_audio_vae(vae_id, sample_rate, latent_sr, window_length):
    ...
    if vae_id == 'stable_audio':
        ...
        wrapper_state.vae = pipe.vae.eval()
        ...
        from diffusers import AutoencoderModel
        wrapper_state.vae = AutoencoderModel.from_pretrained("same-l")
        wrapper_state.vae.autoencoder.bfloat16().eval()
    elif vae_id == 'ltx2':
...
@torch.no_grad()
def encode_audio(wrapper_state, raw_audio):
...
@torch.no_grad()
def decode_audio(wrapper_state, latents):
    vae_id = wrapper_state['vae_id']
    sample_rate = wrapper_state['sample_rate']
    vae = wrapper_state['vae']
