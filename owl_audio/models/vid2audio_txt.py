...
class AudioDiffusionCore(nn.Module):
    ...
    def forward(self, x, ts, video=None, video_tokens=None, text_tokens=None, attn_mask=None):
        # x           : [B, C, T]
...
class AudioDiffusionModel(nn.Module):
    ...
    def forward(self, x, video=None, text=None, attn_mask=None):
        ...
        # Encode captions to text tokens
        text_tokens = None
        if text is not None and self.core.text_proj is not None:
            text_tokens = self.core.text_proj(text)   
